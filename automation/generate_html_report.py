...
# CUSTOMIZE: lowercase substrings that mark a listing as local to your area,
# e.g. ["austin", "texas", "tx "]. Matching listings get a highlighted
# "local" tag in the report.
LOCAL_LOCATION_TERMS = ["[your city]", "[your state]"]
...
def location_tag(loc):
    if not loc:
        return ""
    loc_lower = loc.lower()
    if any(x in loc_lower for x in LOCAL_LOCATION_TERMS):
        return f'<span class="tag local">{escape(loc)}</span>'
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .tag.local {{ background: #d4edda; color: #0d5524; font-weight: 700; }}
